...
static void addNodeInfo(FILE* file, int index, treeNode_t* node) {
    fprintf(file, "Node_%d [shape=Mrecord; style=filled; fillcolor = \"#56db70\"; color = \"black\"; label = <{ptr: <font color=\"%s\">%p</font> | %s | ",
            index, getPointerColor(node), node, getData(node));

    if (getLeft(node) != NULL) {
        fprintf(file, "{L: <font color=\"%s\">%p</font>", getPointerColor(getLeft(node)), getLeft(node));
    }
    else {
...
static void fillHtmlHeader(const char *desc, const char *fileName, const int line, const char *func, error_t code, FILE* htmlFile, size_t counter) {
    assert(htmlFile);
    // заголовок
    fprintf(htmlFile, "</p><h3>DUMP#%llu <font color=red>%s</font> %s at (%s:%d) - STATUS: %d</h3></p>\n",
...
error_t treeDump(treeNode_t* node, const char* desc, const char* file, const int line, const char* func, error_t code) {
    ...
    // Генерируем PNG
    char command[512];
    snprintf(command, sizeof(command), "dot -T svg %s -o %s", DOT_FILE_PATH, svg_filename);
    int result = system(command);

    if (result != 0) {
        fprintf(html_file, "<p style=\"color: red;\">Graphviz generation failed!</p>\n");
    } else {
        fprintf(html_file, "<img src=\"images/tree_%zu.svg\" alt=\"List structure visualization\">\n", counter);
    }
